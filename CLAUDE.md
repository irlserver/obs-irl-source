...
| `av_skew.rs` | The audio hold for a sender whose video is stamped behind its audio: the part of the measured skew that Target Buffer and the output lead do not cover, capped. Pure arithmetic; the plugin half is `irl-source/src/audio/av_skew.rs`. |
...
| `audio/av_skew.rs` | New in 2.x: the receiver measures each video packet's PTS against the newest audio PTS decoded before it, and before priming raises the published watermarks (the user's Target Buffer plus the hold) through `config::publish_watermarks`; the pump's prime gate waits a bounded time for that first measurement when the connection carries video; a new connection resets it. `Config::apply_hot` composes a Target Buffer edit with the hold in force. |
...
`update` diffs the new settings against the live config: URL, FFmpeg Options, Hardware Decode, Low Latency Audio and Keep Lip Sync When Video Arrives Late are latched at stream open and force a reconnect; everything else is swapped in place through `Config::apply_hot`, so a settings tweak neither drops the connection nor clears the stats counters. Retuning Target Buffer live goes through `AudioBuffer::resize`, which grows the ring (never shrinks it) and only then publishes the new watermarks — if the resize fails the old target stays in force, including in the OBS-thread config that the next diff compares against.
...
2. The `video_decoder_flushes` stat is gone (it was always 0 after the video decoder stopped being flushed). 27 stat fields remained; deviations 16 and 17 add `video_delay_ms`, `av_skew_ms` and `audio_hold_ms`, making 30.
...
O. A sender whose video is stamped behind its audio has the audio held back to match (`irl_core::av_skew`, `audio/av_skew.rs`, the Keep Lip Sync When Video Arrives Late setting, on by default). The C mapped video through the audio playout and handed late frames over on arrival, so a phone with video stabilization on — which sends each frame a second or more after the audio of the same instant, both stamped with the capture time (#33) — played with the sound that far ahead of the picture, while the media source, pacing both streams on their PTS, played it in sync. The port measures the skew in mux order as the first video packets arrive and, before priming, raises the connection's jitter-buffer target by the uncovered part of it, so audio starts later together with the picture and video maps with an ordinary margin. `audio_hold_ms` reports the hold, `av_skew_ms` the raw stamp difference; 30 stat fields. Measured only before priming and only ever raised within a connection; a skew that appears afterwards is left to the standing video delay of deviation 16, whose ceiling is 5 s rather than the 1 s the port first shipped with, since the delay costs nothing to carry and a 1 s ceiling turned every such stream into an unpaced one.
