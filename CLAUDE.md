...
| `video_delay.rs` | The standing video delay: sized from the worst shortfall seen so that no frame reaches libobs after its due time, raised at once before the play head is anchored (measured at packet arrival, with a tick's allowance for decode) and only for lateness that recurs across a window after it (measured at hand-over), capped, never lowered within a connection. |
