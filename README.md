...
| FFmpeg Options | | Extra options for the stream reader, `key1=val1 key2=val2` style. Use this to set the SRT `latency`, for example, or `mode=listener` to have the source wait for the sender to call in (see below) |
...
| Keep Lip Sync When Video Arrives Late | On | Some senders stamp their video behind their audio: a phone with video stabilization on sends each frame a second or more after the sound of the same moment. Played as it arrives, the sound would run that far ahead of the picture. With this on, the plugin measures the skew at connection start and holds the audio back by the uncovered part of it, so the two start together and stay together. The hold shows as `audio_hold_ms` and adds that much delay to the audio only; the picture is not delayed at all, it simply cannot arrive sooner. Turn it off to hear audio as soon as it arrives regardless. No effect with Low Latency Audio on |
...
Target Buffer, Reconnect Delay, Adaptive Latency Control, Catch-Up Speed, Wait for Keyframe, Show Nothing When the Stream Ends and Close Stream When Inactive can be changed while the stream is running. The connection stays up and the stats counters keep counting. The one exception is turning Close Stream When Inactive on while the source is already hidden, which is a request to stop receiving: that drops the connection and resets the stats counters, as it would on any later hide. Changing Target Buffer mid-stream keeps every buffered sample and walks the latency to the new value at up to the Catch-Up Speed or -2%, so you should not hear a seam. Changing URL, FFmpeg Options, Hardware Decode, Low Latency Audio or Keep Lip Sync When Video Arrives Late reconnects, because those are set when the stream is opened.

#### Receiving directly from the encoder (listener mode)

To have the source wait for the sender instead of dialing a server, give it a listening URL such as `srt://0.0.0.0:7654` and tell SRT to listen: either `srt://0.0.0.0:7654?mode=listener` in the URL, or `mode=listener` in FFmpeg Options. Both reach the SRT library the same way. While no sender is connected the source simply waits, for hours if need be, with no timeout. The 10 second I/O stall deadline only starts once a sender has connected. The log says `Listening for the sender to call in` when the source is in this state, which is the line to look for if a listener seems to reconnect on its own.
...
| `video_delay_ms` | int | Standing delay added to the video schedule because video reached the plugin too late to be paced against the audio playout (the encoder sends video later than audio by more than Target Buffer covers). Lip sync is off by this much if the sound runs ahead of the picture; a Sync Offset of this much on the source, or raising Target Buffer by at least this, takes it back to zero |
| `av_skew_ms` | int | Video PTS minus audio PTS of what last reached the plugin: how the sender stamps its two streams against each other, before any buffering here. Near zero for a healthy sender; a standing negative value is video stamped behind its audio, which is what `video_delay_ms` then covers |
| `audio_hold_ms` | int | How much longer than Target Buffer audio is held this connection so that video stamped behind it stays in sync (Keep Lip Sync When Video Arrives Late). Sized once from the first video packet, before audio starts; 0 for a sender whose skew Target Buffer already covers |
